...
def load_data_and_labels_fewshot():
    ...
    word_embeddings = []
    word2id = {}
    f = open(vec_path, "r",encoding='ISO-8859-1')
    content = f.readline()
    while True:
        content = f.readline()
        if content == "":
    ...
    f.close()
    # 补充两个字符分别为全0向量；一个是Unknown字符，一个是填充用的空白符
    ...
    namehash = {}
    num_r = {}
    farticlemap = open(law_attribution_path, 'r',encoding="ISO-8859-1")
    ...
    y_attr_test = []
    la = 0
    f = open(test_path, "r",encoding='ISO-8859-1')
    content = f.readlines()
    for i in content:
        z = i.strip().split("\t")
        if(len(z) != 3):
    ...
    y_attr_val = []
    la = 0
    f = open(val_path, "r",encoding='ISO-8859-1')
    content = f.readlines()
    for i in content:
        z = i.strip().split("\t")
        if(len(z) != 3):
